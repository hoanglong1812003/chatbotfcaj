...
def load_vectorstore():
    embeddings = HuggingFaceEmbeddings(
        model_name="sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2", cache_folder="/tmp/huggingface"
    )
...
@st.cache_resource
def setup_rag_chain():
    llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0.1, groq_api_key=os.getenv("GROQ_API_KEY"))
...
"""
    ...
    prompt = ChatPromptTemplate.from_messages(
        [("system", SYSTEM_PROMPT), ("human", "Thông tin:\n{context}\n\nCâu hỏi:\n{question}")]
    )
...
st.set_page_config(
    ...
    menu_items={"Get Help": "https://rules.fcjuni.com/", "About": "# FCAJ Chatbot v1.0"},
)
